// pages/setpage/equadd/equadd.js
Page({
 ...
 stringToBuffer(str) {
  ...
  return buffer;
     ...
     */
    connect() {
      const { location, deviceId } = this.data;
      // 验证昵称是否为空
      if (!location) {
        return wx.showToast({ title: '请输入设备位置', icon: 'none' });
      }

      // 构造位置配置指令（格式参考WiFi的指令逻辑）
      const msg = `LOC:${location}`;
      ...
      wx.writeBLECharacteristicValue({
        ...
        fail: (err) => {
          wx.hideLoading();
          console.error('位置配置发送失败', err);
          wx.showToast({ title: '发送失败', icon: 'error' });
        }
      });
      wx.showLoading({ title: '保存位置中...', mask: true });
      ...
      wx.onBLECharacteristicValueChange((res) => {
        ...
        if (value === "LOC_OK") {
          wx.hideLoading();
          wx.showToast({ title: '位置保存成功' });
          // 延迟返回上一页，保证用户能看到提示
          setTimeout(() => wx.navigateBack(), 1500);
        } else if (value === "LOC_FAIL") {
          wx.hideLoading();
          wx.showModal({ title: '失败', content: '位置保存失败，请重试', showCancel: false });
        }
      });
